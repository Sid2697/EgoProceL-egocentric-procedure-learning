...
class Embedder(nn.Module):
    def __init__(self, embedding_size=128, num_context_steps=2, cfg=None):
        super(Embedder, self).__init__()

        self.num_context_steps = num_context_steps
        self.cfg = cfg

        base_model_name = self.cfg.TCC.BASE_MODEL_NAME.lower()
        if base_model_name == 'resnet50':
            self.base_model = resnet50(pretrained=self.cfg.TCC.PRETRAINED)
        ...
        if self.cfg.TCC.INPUT_SIZE[0] == 224:
            ksize = 14
        elif self.cfg.TCC.INPUT_SIZE[0] == 168:
            ksize = 11
        else:
            raise NotImplementedError

        self.maxpool = nn.MaxPool3d(
            (self.num_context_steps, ksize, ksize)
        )
